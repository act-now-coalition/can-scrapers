...
pytest -v -k Wyoming .
    ...
    To run integration tests, first follow the steps above to build and start a
    docker

    Then you need to set the environment variable =PG_CONN_STR=. To do this in a
    unix environment (Linux or OSX), run the following command

    #+begin_src
export PG_CONN_STR="postgresql://postgres:postgres_test@localhost:5432/covid"
    #+end_src

    If you are using Windows, set the environment variable by running

    #+begin_src
set PG_CONN_STR="postgresql://postgres:postgres_test@localhost:5432/covid"
    #+end_src
