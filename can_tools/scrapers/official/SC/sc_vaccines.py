...
class SCVaccineDemographics(StateDashboard):
    ...
    def normalize(self, data:pd.DataFrame) -> pd.DataFrame:
        ...
        # format data
        cols = {'Age Bins SIMON1-alias':'age', 'AGG(Count individuals with Suppression  for bar graph)-alias':'value', 'Assigned Race-alias':'race', 'recip sex (Recipient Data Vaccine Dashboard v1.csv)-alias':'sex', 'location_name':'location_name'}
        df = df[cols.keys()]
        df = (
            df.rename(columns=cols)
            .replace({'65+':'65_plus', 'M':'male', 'F':'female','Asian/NHPI/AIAN*':'asian'})
            .assign(
