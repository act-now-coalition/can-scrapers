...
class DelawareCountyVaccine(DelawareStateVaccine):
    ...
    def _fetch_fully_vaccinated(self):
        ...
        for curl in fully_vaccinated_urls:
            ...
            first_dose_df = pd.DataFrame(data=first_dose_data, columns=['FirstDose'], index=idx)
            # Get second dose data
            second_dose_data = tdata['series'][1]['data']
            idx = pd.date_range(startDate, periods=len(second_dose_data), freq='d')
            second_dose_df = pd.DataFrame(data=second_dose_data, columns=['SecondDose'], index=idx)
        ...
        for curl in total_urls:
            ...
            for srs in tdata["series"]:
                if srs['name'] == 'Daily Count':
                    ...
                    total_df = pd.DataFrame(data=total_data, columns=['TotalDoses'], index=idx)
            if total_df is None:
    ...
    def normalize(self, data):
        ...
        df = totals.set_index(['dt', 'location_name']).join(doses.set_index(['dt', 'location_name'])).reset_index()
