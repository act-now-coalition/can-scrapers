...
class SCVaccineDemographics(StateDashboard):
    ...
    def fetch(self):
        ...
        counties = [t["values"] for t in filters if t["column"] == "Recipient County for maps"][0]
    ...
    def normalize(self, data: TableauScraper):
        rename = {"AGG(Count individuals with Suppression )-alias": "value", "Assigned Race-value": "race", "ATTR(recip sex (Recipient Data Vaccine Dashboard v1.csv))-alias": "sex", "location_name":"location_name", "Age Bins SIMON-value": "age" }
        return (
            ...
            .assign(
                ...
                race=lambda row: row["race"].replace("Asian, American Indian or Alaskan Native, Native Hawaiian or Other Pacific Islander", "ai_an_asian_or_pacific_islander").str.lower(),
                age=lambda row: row["age"].str.replace("+", "_plus")
            )
            .pipe(self.extract_CMU, cmu=self.variables, var_name="variable", skip_columns=["age","race","sex"])
        )