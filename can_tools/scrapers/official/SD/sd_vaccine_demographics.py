...
class SDVaccineAge(MicrosoftBIDashboard):
    ...
    def fetch(self):
        ...
        for i in range(0, len(counties), 3):
            print("making call for: ", counties[i : i + 3])
    ...
    def normalize(self, resjson):
        ...
        dfs = []
        demo_key = {'0':'female', '1':'male', '2':'unknown'}
        for key, value in demo_key.items():
            col_mapping = {
                "G0": "county",
            ...
            county = pd.DataFrame.from_records(data_rows).assign(sex=value)
        ...
        df["total_vaccine_initiated"] = (
            df["jj"] + df["mod_1_dose"] + df["pf_1_dose"]
        )
