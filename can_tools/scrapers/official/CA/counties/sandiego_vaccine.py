...
class CASanDiegoVaccine(ArcGIS):
    ...
    def normalize(self, data):
        ...
        df = self.arcgis_jsons_to_df(data).rename(
            columns={"Catefory": "Category"}
        )
        ...
        # Reshape
        df = df.melt(
            id_vars=["dt", "location", "Category", "Join_Name"],
            value_vars=["Count_"]
        )
        ...
        eth_df = self.normalize_group(df, eth_col, "ethnicity", ethnicity_replace, crename)
        ...
        total_cols = [
            "IndividualsVacc", "FullyVacc", "DosesShipped", "AdministeredDoses"
        ]
        total_temp = df.query(
            "(Category in @total_cols) & (variable == 'Count_')"
        )
        total_df = self.extract_CMU(
            total_temp, total_crename, var_name="Join_Name"
        ).drop(
            ["Category", "Join_Name", "variable"], axis="columns"
        )

        out = pd.concat(
            [age_df, gender_df, race_df, eth_df, total_df],
