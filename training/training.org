...
pytest -v -k Wyoming .
   ...
   If you are working on a new scraper, you need to stage any new/added files
   in order for the tests to be picked up by the testing suite, (e.g. with =git add WY/=)  
